(async function () {
    ...
    const list = document.querySelector(".emp-list-item");
    ...
    const toast = (msg, type = 'success') => {
        ...
        t.innerHTML = `<span>${type === 'success' ? 'mark' : type === 'warning' ? '' : 'x' }</span> <span>${msg}</span>;
...
list.innerHTML = filtered.length ? filtered.map(e => `
    ...
    <span><i class="emp-edit"></i><i class="emp-delete">x</i></span>
...
const emp = employees.find(e => e.id === selectedId);
    info.innerHTML = emp ?
        ...
        <button class="delete-detail-btn" onclick="delEmp(${emp.id})">x Delete</button>
    ...
    const emp = { id: Math.max(...employees.map(e => e.id), 1000) + 1, ...Object.fromEntries(f), age: 2026 - +f.get ('dob').slice(0,4), imageUrl:
    f.get('imageUrl') || `https://1.pravatar.cc/150?img=$ {Math.floor(Math.random()*70)}`};
    employees.push(emp);
 ...
 window.openEdit = id => {
    ...
    editForm.editFirstName.value = e.fisrtName;
...
editForm.onsubmit = e => { e.preventDefault();
    ...
    employees[i] = { ...employees[i], firstName: f.get("editFirstName"), 
    lastName: f.get("edit:LastName"), email: f.get("editEmail"), contactNumber:
    f.get("editContactNumber"), salary: f.get("editSalary"), address: f.get("editAddress"), 
    dob: `${d}-${m}-${y}`, age: 2026 - +y };
...
list.onclick = e => {
    ...
    { delEmp(+e.target.closest(".emp-item").id); return; }
    if (e.target.classList.contains("emp-edit"))
    { openEdit(+e.target.closest(".emp-item").id); return; }
